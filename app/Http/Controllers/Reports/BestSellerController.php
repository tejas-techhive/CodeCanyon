...
class BestSellerController extends Controller
{
    public function index(Request $request, $author_name = null)
    {
        ...
        if (!empty($site) && !empty($featured_type)) {

            $items->where('site', $site)->where('featured_type', $featured_type);
        } elseif (!empty($site)) {

            $items->where('site', $site);
        } elseif (!empty($featured_type)) {

            $items->where('featured_type', $featured_type);
        }
        ...
        return view('main_reports.best-sellers', compact('items', 'searchTerm', 'sortOrder', 'startDate', 'endDate'));
    }
}
