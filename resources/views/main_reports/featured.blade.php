...
@section('content')
    ...
    <div class="container mt-5">
        ...
        <h1 class="text-center mb-4"> Featured Report</h1>

        <!-- Search and Category Filter Form -->
        <form action="{{ route('reports.best-sellers') }}" method="GET" class="mb-4">
            <div class="row">
        ...
        <div class="row">
            ...
            <div class="col-md-2">
                <select name="featured_type" id="featured_type" class="form-select">
                    <option value="" {{ request('sites') == '' ? 'selected' : '' }}>Select featured type</option>
                    <option value="featured-themes" {{ request('featured_type') == 'featured-themes' ? 'selected' : '' }}>
                        Featured Themes
                    </option>
                    <option value="featured-creater" {{ request('featured_type') == 'featured-creater' ? 'selected' : '' }}>
                        Featured Creater
                    </option>
                    <option value="featured-code" {{ request('featured_type') == 'featured-code' ? 'selected' : '' }}>
                        Featured Code
                    </option>
                </select>
            </div>
...
@section('scripts')
    ...
    <script>
        ...
        document.getElementById('featured_type').addEventListener('change', function() {
            const selectedValue = this.value;
            const url = new URL(window.location.href);
            url.searchParams.set('featured_type', selectedValue);
            window.location.href = url.toString();
        });
    </script>
@endsection
