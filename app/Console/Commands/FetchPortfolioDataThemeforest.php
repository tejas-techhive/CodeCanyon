...
class FetchPortfolioDataThemeforest extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'portfoliothemeforest:fetch';
    ...
    {
        $authors = AuthorThemeForest::select('id', 'name')
            ->where('is_complete', 0)
            ->take(6)
            ->get();
        ...
        $requests = function ($authors) {
            foreach ($authors as $author) {
                $url = "https://themeforest.net//user/{$author->name}/portfolio?order_by=sales";
        ...
        foreach (array_chunk($results, 100) as $batch) {
            ...
            PortfolioItemThemeForest::insert($batch);
        }

        $this->info('Portfolio data of Themeforest successfully fetched and stored.');
    }
}
